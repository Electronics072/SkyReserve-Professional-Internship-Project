...
public class BookingService {
    ...
    public List<Booking> mine(String email){return bookings.findByUserEmailOrderByCreatedAtDesc(email);} public Optional<Booking> byRef(String ref){return bookings.findByBookingReference(ref);}
    public Set<String> occupiedSeats(Long scheduleId){Set<String> result=new HashSet<>();bookings.findByScheduleIdAndStatus(scheduleId,BookingStatus.CONFIRMED).forEach(b->result.add(b.getSeatNumber()));return result;}
    public long count(){return bookings.count();} public long confirmed(){return bookings.countByStatus(BookingStatus.CONFIRMED);} public long cancelled(){return bookings.countByStatus(BookingStatus.CANCELLED);} public double revenue(){return bookings.confirmedRevenue();} public List<Booking> all(){return bookings.findAll();}
    @Transactional public void cancel(String ref,String email,boolean admin){Booking b=bookings.findByBookingReference(ref).orElseThrow(()->new IllegalArgumentException("Booking not found"));if(!admin&&!b.getUser().getEmail().equalsIgnoreCase(email))throw new SecurityException("You are not allowed to cancel this booking.");if(b.getStatus()==BookingStatus.CANCELLED)return;b.setStatus(BookingStatus.CANCELLED);bookings.save(b);}
}
