...
public class BookingController {
    ...
    public BookingController(FlightRepository f, FlightScheduleRepository s, BookingService b,
                             RazorpayService r, OtpService o) {
        flights = f; schedules = s; service = b; razorpay = r; otpService = o;
    }
    ...
    String form(@RequestParam Long flightId, @RequestParam(required = false) LocalDate date, Model m) {
        ...
        m.addAttribute("seats", generateSeats(f.getTotalSeats()));
        m.addAttribute("occupiedSeats", service.occupiedSeats(sc.getId()));
        return "booking";
    }

    @PostMapping("/checkout")
    String checkout(@RequestParam Long scheduleId, @RequestParam String passengerName,
                    @RequestParam String seatNumber, @RequestParam String seatClass, Model m) {
        ...
        double fare = "Business".equalsIgnoreCase(seatClass)
                ? sc.getFlight().getBusinessPrice() : sc.getFlight().getEconomyPrice();
        double fee = Math.round(fare * .05 * 100.0) / 100.0;
        m.addAttribute("fare", fare);
        m.addAttribute("serviceFee", fee);
        m.addAttribute("total", fare + fee);
        m.addAttribute("razorpayEnabled", !razorpay.getKeyId().isBlank());
        return "payment";
    }

    @PostMapping("/razorpay/otp/send")
    @ResponseBody Object sendPaymentOtp(@RequestParam Long scheduleId, @RequestParam String passengerName,
                                        @RequestParam String seatNumber, @RequestParam String seatClass,
                                        Authentication auth, HttpSession session) {
        try {
            schedules.findById(scheduleId).orElseThrow(() -> new IllegalArgumentException("Schedule not found"));
            session.setAttribute("rzScheduleId", scheduleId);
            session.setAttribute("rzPassengerName", passengerName.trim());
            session.setAttribute("rzSeatNumber", seatNumber.trim().toUpperCase());
            session.setAttribute("rzSeatClass", seatClass);
            session.setAttribute("rzEmail", auth.getName());
            String demoOtp = otpService.sendPaymentOtp(auth.getName(), session);
            String masked = maskEmail(auth.getName());
            Map<String, Object> result = new java.util.HashMap<>();
            result.put("success", true);
            result.put("message", "OTP sent to " + masked);
            if (demoOtp != null) result.put("demoOtp", demoOtp);
            return result;
        } catch (Exception e) {
    ...
    @ResponseBody Object verifyPaymentOtp(@RequestParam String otp, HttpSession session) {
        try {
            otpService.verifyPaymentOtp(otp, session);
            return Map.of("success", true, "message", "OTP verified. You can now continue to Razorpay.");
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(Map.of("success", false, "error", e.getMessage()));
        }
    }
    ...
    @ResponseBody Object createRazorpayOrder(Authentication auth, HttpSession session) {
        try {
            if (!otpService.isVerified(session)) throw new IllegalStateException("Verify the payment OTP first.");
            ...
            if (scheduleId == null || passenger == null || seat == null || seatClass == null || email == null
                    || !email.equalsIgnoreCase(auth.getName())) {
                throw new IllegalStateException("Payment session expired. Please start checkout again.");
            }
            FlightSchedule sc = schedules.findById(scheduleId).orElseThrow(() -> new IllegalArgumentException("Schedule not found"));
            double fare = "Business".equalsIgnoreCase(seatClass)
                    ? sc.getFlight().getBusinessPrice() : sc.getFlight().getEconomyPrice();
            double fee = Math.round(fare * .05 * 100.0) / 100.0;
            Order order = razorpay.createOrder(fare + fee, "SR-" + System.currentTimeMillis());
            session.setAttribute("rzOrderId", order.get("id"));
            return Map.of("key", razorpay.getKeyId(), "orderId", order.get("id"),
                    "amount", order.get("amount"), "currency", order.get("currency"),
                    "name", "SkyReserve", "description", "Flight reservation payment");
        } catch (Exception e) {
    ...
    @ResponseBody Object verifyRazorpayPayment(@RequestParam String razorpay_order_id,
                                                @RequestParam String razorpay_payment_id,
                                                @RequestParam String razorpay_signature,
                                                Authentication auth, HttpSession session) {
        try {
            if (!otpService.isVerified(session)) throw new IllegalStateException("Payment OTP verification is required.");
            String serverOrderId = (String) session.getAttribute("rzOrderId");
            if (serverOrderId == null || !serverOrderId.equals(razorpay_order_id)) {
                throw new IllegalStateException("Invalid payment order.");
            }
            if (!razorpay.verify(serverOrderId, razorpay_payment_id, razorpay_signature)) {
            ...
            return Map.of("success", true, "bookingReference", b.getBookingReference(), "paymentId", razorpay_payment_id);
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(Map.of("success", false, "error", e.getMessage()));
        }
    }
    ...
    @PostMapping("/create")
    String create(@RequestParam Long scheduleId, @RequestParam String passengerName,
                  @RequestParam String seatNumber, @RequestParam String seatClass,
                  @RequestParam String paymentMethod, Authentication auth, Model m) {
        try {
    ...
    @GetMapping
    String mine(Authentication a, Model m) { m.addAttribute("bookings", service.mine(a.getName())); return "bookings"; }
    ...
    String cancel(@PathVariable String ref, Authentication a) {
        ...
        service.cancel(ref, a.getName(), admin); return "redirect:/bookings";
    }
    ...
    ResponseEntity<ByteArrayResource> pdf(@PathVariable String ref, Authentication auth) throws Exception {
        ...
        Document document = new Document(); PdfWriter.getInstance(document, out); document.open();
        ...
        return ResponseEntity.ok().header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=SkyReserve-" + b.getBookingReference() + ".pdf")
    ...
    private List<String> generateSeats(int totalSeats) {
        List<String> seats = new ArrayList<>(); String[] columns = {"A", "B", "C", "D", "E", "F"};
